...
type
  TFrmPrincipal = class(TForm)
    ...
    procedure layout_menu_comproClick(Sender: TObject);
...
uses cLancamento, UnitDM, UnitLancamentosCad, cUsuario, UnitLogin,
  System.DateUtils, cCompromissos, UnitCompromissos;
...
begin
  try
    ...
    if qry.FieldByName('FOTO').AsString <> '' then
      foto := qry.CreateBlobStream(qry.FieldByName('FOTO'),
...
begin
  try
    ...
    // Somamos pq o vl_desp já esta negativo...
...
procedure TFrmPrincipal.Enviar_Notificacao(nome, titulo, texto : string);
var
...
begin
  ...
  if tempo = tempo2 then
      with FrmPrincipal do
      begin
           Enviar_Notificacao('Fisioterapia','Compromisso','Rua dalo, 152')
      end;
end;
...
procedure TFrmPrincipal.AddCategoria(ListView: TListView;
  id_categoria, categoria: string; foto: TStream);
var
  txt: TListItemText;
  img: TListItemImage;
  bmp: TBitmap;
...
begin
  try
    ...
    begin
      ...
      AddLancamento(FrmPrincipal.lv_lancamento, qry.FieldByName('ID_LANCAMENTO')
        .AsString, qry.FieldByName('DESCRICAO').AsString,
        qry.FieldByName('DESCRICAO_CATEGORIA').AsString,
        qry.FieldByName('VALOR').AsFloat, qry.FieldByName('DATA')
        .AsDateTime, foto);
...
procedure TFrmPrincipal.layout_menu_comproClick(Sender: TObject);
begin
  AnimationMenu.Start;

  if NOT Assigned(FrmCompromissos) then
    Application.CreateForm(TFrmCompromissos, FrmCompromissos);

  FrmCompromissos.Show;
end;
